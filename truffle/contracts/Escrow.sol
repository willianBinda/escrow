...
contract Escrow is AccessControl{
    ...
    enum StatusItem { PENDENTE, FINALIZADO }
    ...
    event ItemAdicionado(address indexed fornecedor, uint256 itemId, string descricao, uint256 valor, uint256 quantidade);
    event ItemComprado(address indexed comprador, address indexed fornecedor, uint256 itemId, uint256 valorPago);
    ...
    function concederPermissaoFornecedor(address _conta) public onlyRole(DEFAULT_ADMIN_ROLE){
        _grantRole(FORNECEDOR_ROLE, _conta);
    }

    function removerPermissaoFornecedor(address _conta) public onlyRole(DEFAULT_ADMIN_ROLE){
    ...
    function adicionarItem(
        ...
        uint256 _valor, 
        uint256 _quantidade) public onlyRole(DEFAULT_ADMIN_ROLE){
        ...
        emit ItemAdicionado(msg.sender, produtoId, _descricao, _valor, _quantidade);
    }

    function listarItens(address _fornecedor) public view returns (Item[] memory) {
    ...
    function comprarItem(address _fornecedor, uint256 _itemId) external payable {
        ...
        require(msg.sender != _fornecedor, "Fornecedor nao pode comprar seu proprio item");
    ...
    function confirmarRecebimento(uint256 _pedidoIndex) external {
        require(_pedidoIndex < itensComprados[msg.sender].length, "Pedido inexistente");
